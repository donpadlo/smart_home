...
foreach ($data as $dev){  
    ...
    if (isset($dev_info["IP"])):
        $url = "http://".$dev_info["IP"].":8081/zeroconf/info";
        echo "$url\n";
        $post_data = '{"deviceid": "'.$dev_info["deviceid"].'","data": { }}';
        echo "$post_data\n";
        $ch = curl_init();
        ...
        $switch=json_decode($output->data);
        $sw=$switch->switch;
        $sql="insert into storage (id,dt,device,datatype,value) values (null,now(),$device_id,3,'$sw')";        
        echo "$sql\n";
        $stmt3=$sqln->dbh->prepare($sql);
        $stmt3->execute();
        //$sqln->dbh->commit();
